...
def process_folder(base_dir, folder_name, prompt_template):
    ...
    # 查找baseline文件夹中的所有 C 文件（这些是需要处理的目标文件）
    target_files = glob.glob(os.path.join(baseline_folder, "*.c"))
    
    if not target_files:
        print(f"No target files found in {baseline_folder}")
        return
    ...
    for target_file_path in target_files:
        ...
        print(f"Saved optimized C file to {target_file_path}")
